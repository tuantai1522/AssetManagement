...
{
	public class CompleteReturnRequestByIdAsync : SetupReturningRequestServiceTest
	{
		public CompleteReturnRequestByIdAsync() : base() { }
		[Fact]
		...
		public async Task CompleteReturnRequestByIdAsync_Success()
		{
			...
			var request = _returningRequests.First(r => r.State == ReturningRequestState.WaitingForReturning);
			...
			// Assert
			Assert.Equal(ReturningRequestState.Completed, request.State);
			Assert.Equal(AssignmentState.Returned, request.Assignment.State);
			Assert.Equal(AssetState.Available, request.Assignment.Asset!.State);
			Assert.Equal(userId, request.AcceptedByUserId);
