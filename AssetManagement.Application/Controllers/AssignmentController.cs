...
public class AssignmentController : ControllerBase
{
    ...
    [Route("respond/{id}")]
    [Authorize]
    public async Task<ActionResult> RespondAsync(string id, [FromBody] RespondAssignmentRequest request)
    {
        var data = await _assignmentService.RespondAsync(id, request);
        var result = new BaseResult<List<RespondAssignmentResponse>>()
        {
            IsSuccess = true,
            Error = null,
            Result = [new RespondAssignmentResponse()]
        };
        return Ok(result);
    }
}
